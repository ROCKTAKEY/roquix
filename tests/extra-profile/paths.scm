...
(call-with-temporary-directory (lambda (directory)
                                 (let* ((root (string-append directory
                                   ...
                                   (let ((configured (resolve-profile name
                                                                      ...
                                                                      store)))
                                     (test-assert
                                      "profile symlinks resolve to a configured-profile value"
                                      (configured-profile? configured))
                                     (test-equal
                                      "profile name survives the state transition"
                                      "profile-a"
                                      (profile-name-value (configured-profile-name
                                                           configured)))
                                     (test-equal
                                      "generation target is snapped to the terminal store path"
                                      target
...
(call-with-temporary-directory (lambda (directory)
                                 (let* ((root (string-append directory
                                        ...
                                        (store (string-append directory
                                                              "/store"))
                                        (store-alias (string-append directory
                                                      "/store-alias"))
                                        (name (parse-profile-name "profile-a"))
                                        (profile (profile-path name
                                                               #:root root))
                                        (target (make-store-profile store
                                                 "aaaaaaaa-profile-a")))
                                   (mkdir-p (dirname profile))
                                   (symlink store store-alias)
                                   (symlink (string-append store-alias "/"
                                                           (basename target))
                                            profile)
                                   (test-equal
                                    "symlinks in terminal path components are canonicalized"
                                    target
                                    (configured-profile-generation-target (resolve-profile
                                                                           name
                                                                           #:profiles-root
                                                                           root
                                                                           #:store-directory
...
(call-with-temporary-directory (lambda (directory)
                                 (let* ((root (string-append directory
                                   ...
                                   (mkdir-p store)
                                   (test-equal
                                    "a missing profile is not configured"
                                    'not-configured
                                    (condition-kind (lambda ()
                                   ...
                                   (test-equal
                                    "a broken profile symlink is distinguished"
                                    'broken-symlink
                                    (condition-kind (lambda ()
                                                      (resolve-profile name
                                                                       #:profiles-root
                                   ...
                                   (call-with-output-file profile
                                     (const #t))
                                   (test-equal
                                    "a regular file is not accepted as a profile"
                                    'invalid-profile
                                    (condition-kind (lambda ()
                                                      (resolve-profile name
                                                                       #:profiles-root
                                   ...
                                   (test-equal
                                    ...
                                    (condition-kind (lambda ()
                                                      (resolve-profile name
                                                                       ...
                                                                       store))))
                                   (delete-file profile)
                                   (let ((target (string-append store
                                                  "/bbbbbbbb-profile-a")))
                                     (mkdir-p target)
                                     (symlink target profile)
                                     (test-equal
                                      "store targets without a manifest are rejected"
                                      'missing-manifest
                                      (condition-kind (lambda ()
                                                        (resolve-profile name
                                                         #:profiles-root root
