...
(define (forwarding-build-option original)
  (option (option-names original)
          ...
          (lambda (option name argument names all? dry-run? forwarded)
  ...
  (call-with-values
      (lambda ()
        (args-fold*
         ...
         (lambda (argument names all? dry-run? forwarded)
           (values (append names (list argument))
                   all? dry-run? forwarded))
         '() #f #f '()))
    (lambda (raw-names all? dry-run? build-arguments)
      (when (and all? (pair? raw-names))
        (leave (G_ "--all cannot be combined with profile names~%")))
      (when (and (not all?) (null? raw-names))
        (raise-extra-profile-error 'missing-name #f #f))
      (make-reconfigure-invocation
...
Manage named extra profiles and compose them into guix shell environments.\n"))
  ...
  (display (G_ "  shell NAME... [-- ARGS...]   run guix shell with extra profiles\n"))
  (display (G_ "  generations NAME [PATTERN]   list profile generations\n"))
  (display (G_ "  roll-back NAME               switch to the previous generation\n"))
  (newline)
  (show-bug-report-information))

(define (require-one-name subcommand arguments)
  (match arguments
    ((name) (parse-profile-name name))
    (_ (leave (G_ "~a requires exactly one profile name~%") subcommand))))

(define (extra-profile-error-reason kind)
  (case kind
    ((broken-symlink) "its generation link is broken")
    ((symlink-cycle) "its generation links contain a cycle")
    ((symlink-limit) "its generation link chain is too deep")
    ((invalid-profile) "its profile path is not a symbolic link")
    ((outside-store) "its current generation is outside the Guix store")
    ((missing-manifest) "its current generation has no manifest")
    ((unsafe-cache-directory) "the shell manifest cache directory is unsafe")
    ((unsafe-cache-file) "the shell manifest cache file is unsafe")
    ((invalid-guix-executable) "the running Guix executable cannot be resolved")
  ...
  (let ((kind (extra-profile-error-kind condition))
    ...
    (case kind
      ...
      ((not-configured)
       ...
       (display-hint
        (format #f (G_ "Run @command{guix extra-profile reconfigure ~a}.~%")
                name))
       (exit 1))
      ((missing-definition)
       (leave (G_ "definition manifest for profile '~a' does not exist: ~a~%")
              name path))
      (else
       (leave (G_ "profile '~a' is unusable: ~a~@[ (~a)~]~%")
              (or name "?") (extra-profile-error-reason kind) path)))))
...
(define (run-generations arguments)
  (match arguments
    ((raw-name)
     (run-generations (list raw-name #f)))
    ((raw-name pattern)
     (let* ((name (parse-profile-name raw-name))
            (profile (resolve-profile name))
            (status (run-guix-operation
  ...
  (match arguments
    ...
    (("show" rest ...)
     (write-profile-description
      (describe-profile (require-one-name "show" rest))
      (current-output-port)))
    (("path" rest ...)
    ...
    (("shell" rest ...)
     (execute-snapshotted-shell
      (snapshot-shell-invocation (parse-shell-arguments rest))))
    (("generations" rest ...)
...
(define-command (guix-extra-profile . arguments)
  ...
  (guard (condition
          ((extra-profile-error? condition)
           (report-extra-profile-error condition)))
    (dispatch arguments)))
