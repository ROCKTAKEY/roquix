     ...
     (list
      ...
      #~(modify-phases %standard-phases
          ...
          (add-after 'patch-opencv-world 'fix-gcc14-set-values
            (lambda _
              (substitute* "src/slic3r/GUI/PhysicalPrinterDialog.cpp"
                (("set_values\\(\\{ m_config->opt_string\\(\\\"printhost_port\\\"\\) \\}\\)")
                 "set_values(std::vector<std::string>{ m_config->opt_string(\"printhost_port\") })"))
              #t)))))
    (native-inputs
